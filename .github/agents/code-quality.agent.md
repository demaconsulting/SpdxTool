...
## SpdxTool-Specific Quality Rules

- **XML Docs**: Enforce on ALL members (public/internal/private) with spaces after `///`
- **Code Style**: Verify `.editorconfig` compliance (file-scoped namespaces, 4-space indent)
- **Test Naming**: Check `SpdxTool_*` pattern for self-validation tests
- **Self-Validation**: Run `dotnet run --project src/DemaConsulting.SpdxTool -- --validate`
...
dotnet buildmark --tools tools.yaml --output docs/buildnotes.md
dotnet reqstream --report docs/requirements/requirements.md \

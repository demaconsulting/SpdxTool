...
string. This scenario is tested by `RenameId_Run_EmptyOldId_ThrowsException`.
...
`RenameId_Run_NewIdAlreadyInUse_ThrowsException`.

**SameId**: the unit performs no document mutation when the old and new IDs are identical. This
scenario is tested by `RenameId_Run_SameId_NoOp`.

...
- **project-name**: SpdxTool
- **organization**: DEMA Consulting
- **project-tagline**: .NET tool for manipulating SPDX SBOM files
- **description**: DemaConsulting.SpdxTool is a .NET tool for creating, validating,
  and manipulating SPDX (Software Package Data Exchange) documents. The repository
  also contains DemaConsulting.SpdxTool.Targets, an MSBuild targets extension that
...
└── test/
    ├── DemaConsulting.SpdxTool.Tests/
    ├── DemaConsulting.SpdxTool.Targets.Tests/
    └── TestFixtures/
```
...
- **`pip-requirements.txt`** - Python dependencies for yamllint and yamlfix <!-- cspell:ignore yamlfix -->
